...
export function expandPackets(model) {
  ...
  for (const f of model.flows) {
    ...
    for (let k = 0; ; k++) {
      ...
      if (rel >= model.cycle_time_us) break;
      pkts.push({
...
export function computeGateSchedule(model, pkts) {
  ...
  for (const lnk of model.links) {
    ...
    if (!schedule[lnk.from]) schedule[lnk.from] = {};
    schedule[lnk.from][lnk.id] = entries;
  }
...
function buildResult(model, pkts, schedHops, method, stats) {
  ...
  for (let p = 0; p < pkts.length; p++) {
    ...
    for (let h = 0; h < rt.hops.length; h++) {
      ...
      hops.push({ link_id: hp.lid, start_us: round3(s), end_us: e, duration_us: round3(hp.tx), queue: assignedQ });
  ...
  for (const pi of order) {
    ...
    for (let ri = 0; ri < pk.routes.length; ri++) {
      ...
      for (let h = 0; h < rt.hops.length; h++) {
        ...
        t = res.t + hp.tx + hp.pd + model.processing_delay_us;
      }

      if (valid) {
        const lastH = rt.hops.at(-1);
        const fin = starts.at(-1) + lastH.tx + lastH.pd;
        if (fin < bestEnd) { bestEnd = fin; bestRoute = ri; bestStarts = starts; bestQueues = queues; }
      }
    }

    if (!bestStarts) {
      const rt = pk.routes[0];
      bestStarts = []; bestQueues = []; let t = pk.rel;
      for (const hp of rt.hops) { bestStarts.push(t); bestQueues.push(-1); t += hp.tx + hp.pd + model.processing_delay_us; }
      bestRoute = 0;
      fallbackCount++;
    }
...
export function renderGCL(model, result, opts = {}) {
  ...
  const activeLinks = model.links.filter(l => {
    ...
    return entries.some(e => !e.note.includes("non-ST"));
  });
  ...
  activeLinks.forEach(link => {
    ...
    entries.forEach(e => {
      if (!e.note.includes("non-ST")) maxActiveEnd = Math.max(maxActiveEnd, e.end_us);
    });
  });
  ...
  activeLinks.forEach(link => {
    ...
    const transitions = visibleEntries.filter(e => !e.note.includes("non-ST"));
    ...
    // Labels on flow bars
    barG.selectAll("text")
      .data(visibleEntries.filter(e => !e.note.includes("non-ST") && !e.note.includes("guard")))
      .enter().append("text")
...
export function renderUtilization(model, result, optsOrId = "utilContainer") {
  ...
  const linkData = model.links.map(l => {
    ...
    entries.forEach(e => {
      if (e.note.includes("non-ST")) beTime += e.duration_us;
...
export function renderSwitchGCL(model, result, opts = {}) {
  ...
  switches.forEach(sw => {
    model.links.filter(l => l.from === sw.id).forEach(l => {
      const entries = result.gcl.links[l.id]?.entries || [];
      entries.forEach(e => {
        if (!e.note.includes("non-ST")) globalMaxActive = Math.max(globalMaxActive, e.end_us);
      });
    });
  });
  const zoomThreshold = model.cycle_time_us * 0.3;
