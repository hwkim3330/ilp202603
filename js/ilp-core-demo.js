...
const FLOW_COLOR_MAP = {
  ...
  "f_lidar_r":   "#6b7280",  // 회색
  "f_radar_r":   "#e11d48",  // 로즈
  "f_bg_small":  "#ec4899",  // 연분홍
  "f_bg_med":    "#f472b6",  // 연분홍(밝은)
  "f_bg_large":  "#f472b6",  // 연분홍(밝은)
};
...
export function getFlowColorsHex() { return FLOW_COLOR_MAP; }
...
function ethFrameBytes(udpData) { return udpData + 20 + 8 + 14 + 4 + 4 + 8 + 12; } // +70: IP(20)+UDP(8)+EthHdr(14)+VLAN(4)+FCS(4)+Preamble(8)+IFG(12)
function txTimeUs(wireBytes, mbps) { return ((wireBytes * 8) / mbps) + 1; }
function isTsn(dl) { return dl !== null; }
function gateMask(p) { const b = Array(8).fill('0'); b[7 - Math.max(0, Math.min(7, p))] = '1'; return b.join(''); }
export function gcd(a, b) { a = Math.round(a); b = Math.round(b); while (b) { [a, b] = [b, a%b]; } return a; }
export function lcm(a, b) { a = Math.round(a); b = Math.round(b); return a / gcd(a, b) * b; }
export function getCycleTime(periods) {
  cycleTime = periods.reduce((acc, cur) => lcm(acc, cur));
  return cycleTime*2;
}
...
export function expandPackets(model) {
  cycleTime = getCycleTime([...new Set(model.flows.filter(f => f.traffic_type !== "background").map(f => f.period_us))]);
  ...
  for (const f of model.flows) {
    ...
    const repsRaw = cycleTime / f.period_us;
    const reps = Math.round(repsRaw);
    //if (Math.abs(repsRaw - reps) > 1e-9) throw new Error(`flow ${f.id}: cycleTime must be divisible by period_us`);
    for (let k = 0; k < reps; k++) {
      const rel = k * f.period_us + (f.rel || 0);
      pkts.push({
        pid: `${f.id}#${k}`, fid: f.id, pri: f.PCP, tt: f.traffic_type,
        rel, dl: f.deadline_us == null ? null : rel + f.deadline_us,
        tsn: isTsn(f.deadline_us),
        routes: cp.map((pl, ri) => ({ ri, hops: pl.map(lid => ({ lid, tx: txTimeUs(ethFrameBytes(f.payload_bytes), lm.get(lid).rate_mbps), pd: lm.get(lid).prop_delay_us })) }))
      });
    }
  }
  return pkts;
...
function buildResult(model, pkts, schedHops, method, stats) {
  ...
  for (let p = 0; p < pkts.length; p++) {
    ...
    pktRows.push({ packet_id: pk.pid, flow_id: pk.fid, priority: pk.pri, selected_route: selR, release_us: round3(pk.rel), end_us: round3(fin), e2e_delay_us: e2e, deadline_abs_us: pk.dl == null ? null : round3(pk.dl), slack_us: pk.dl == null ? null : round3(pk.dl - fin), status: pk.dl == null ? 'NON-ST' : ok ? 'OK' : 'MISS', hops });
  }

  const gcl = { cycleTime : cycleTime, base_time_us: 0, links: {} };
  const nodeType = Object.fromEntries(model.nodes.map(n => [n.id, n.type]));
  // Collect links used by background flows (edge links that need gate open for bg)
  const bgLinkSet = new Set();
  for (const f of model.flows) {
    if (f.traffic_type === 'background' && f.src && f.dst) {
      // Mark all links reachable from bg source
      for (const l of model.links) { if (l.from === f.src) bgLinkSet.add(l.id); }
    }
  }
  for (const lnk of model.links) {
    const isTrunk = nodeType[lnk.from] === 'switch';
    const isBgEdge = !isTrunk && bgLinkSet.has(lnk.id);
    const rows = linkRows[lnk.id].slice().sort((a, b) => a.start_us - b.start_us);
    const entries = []; let cur = 0, idx = 0;
    for (let ri = 0; ri < rows.length; ri++) {
      const r = rows[ri];
      if (r.start_us > cur) {
        const gapStart = round3(cur), gapEnd = round3(r.start_us), gapDur = round3(r.start_us - cur);
        if (isTrunk && r.tsn && gapDur > model.guard_band_us) {
          // Trunk link: non-ST gap + guard band before TSN
          const guardStart = round3(gapEnd - model.guard_band_us);
          entries.push({ index: idx++, gate_mask: '00000001', start_us: gapStart, end_us: guardStart, duration_us: round3(guardStart - gapStart), note: 'BE' });
          entries.push({ index: idx++, gate_mask: '00000000', start_us: guardStart, end_us: gapEnd, duration_us: round3(model.guard_band_us), note: 'guard band' });
        } else if (isTrunk && r.tsn && gapDur > 0) {
          // Trunk link: gap < guard band — no frame can start
          entries.push({ index: idx++, gate_mask: '00000000', start_us: gapStart, end_us: gapEnd, duration_us: gapDur, note: 'closed' });
        } else if (isTrunk || isBgEdge) {
          // Trunk or bg edge link: gate open for BE traffic
          entries.push({ index: idx++, gate_mask: '00000001', start_us: gapStart, end_us: gapEnd, duration_us: gapDur, note: 'BE' });
        } else {
          // Edge link: no traffic, gate closed
          entries.push({ index: idx++, gate_mask: '00000000', start_us: gapStart, end_us: gapEnd, duration_us: gapDur, note: 'BE' });
        }
      }
      entries.push({ index: idx++, gate_mask: gateMask(r.priority), start_us: r.start_us, end_us: r.end_us, duration_us: r.duration_us, note: r.note });
      cur = Math.max(cur, r.end_us);
    }
    if (cur < cycleTime) {
      const remMask = (isTrunk || isBgEdge) ? '00000001' : '00000000';
      entries.push({ index: idx++, gate_mask: remMask, start_us: round3(cur), end_us: round3(cycleTime), duration_us: round3(cycleTime - cur), note: 'BE' });
    }
    gcl.links[lnk.id] = { from: lnk.from, to: lnk.to, entries };
  }

  // Place background packets into non-ST gap windows (best-effort)
  const nonStWindows = {};
  for (const lnk of model.links) {
    nonStWindows[lnk.id] = [];
    for (const e of gcl.links[lnk.id].entries) {
      if ((e.note.includes('non-ST') || e.note === 'BE') && e.gate_mask !== '00000000') {
        nonStWindows[lnk.id].push({ start: e.start_us, end: e.end_us });
      }
    }
  }
  const bgOcc = Object.fromEntries(model.links.map(l => [l.id, []]));

  function findBgSlot(lid, earliest, duration) {
    const windows = nonStWindows[lid], occ = bgOcc[lid];
    for (const w of windows) {
      let t = Math.max(earliest, w.start);
  ...
  const bgPktIndices = [];
  for (let p = 0; p < pkts.length; p++) {
    if (schedHops[p].skipped) bgPktIndices.push(p);
  }
  bgPktIndices.sort((a, b) => pkts[a].rel - pkts[b].rel);

  for (const p of bgPktIndices) {
    ...
    for (let h = 0; h < rt.hops.length; h++) {
      const hp = rt.hops[h];
      const s = findBgSlot(hp.lid, t, hp.tx);
      if (s == null) { console.warn(`[BG-core] ${pk.pid} h=${h}: FAIL on "${hp.lid}", earliest=${round3(t)}, dur=${round3(hp.tx)}, windows=`, JSON.stringify(nonStWindows[hp.lid])); valid = false; break; }
      hops.push({ link_id: hp.lid, start_us: round3(s), end_us: round3(s + hp.tx), duration_us: round3(hp.tx), queue: 7 - pk.pri });
      t = round3(s + hp.tx + hp.pd + model.processing_delay_us);
    }

    if (valid && hops.length > 0) {
      // Commit bg occupancy
      for (let h = 0; h < rt.hops.length; h++) {
        bgOcc[rt.hops[h].lid].push([hops[h].start_us, hops[h].end_us]);
        bgOcc[rt.hops[h].lid].sort((a, b) => a[0] - b[0]);
      }
      // Insert BG packet bars into GCL entries so they appear on the chart                                           
      for (const hp of hops) {                             
        const le = gcl.links[hp.link_id];                  
        if (!le) continue;                                 
        const mask = '00000001'; // TC0                    
        le.entries.push({ index: le.entries.length, gate_mask: mask, start_us: hp.start_us, end_us: hp.end_us, duration_us: hp.duration_us, note: pk.pid });                     
      }
      const fin = round3(hops.at(-1).end_us + rt.hops.at(-1).pd);
      pktRows.push({
  ...
  let worstUtil = 0;
  for (const lnk of model.links) {
    let act = 0; for (const e of gcl.links[lnk.id].entries) if (!e.note.includes('non-ST') && e.note !== 'BE' && !e.note.includes('guard') && e.note !== 'closed') act += e.duration_us;
    worstUtil = Math.max(worstUtil, act / cycleTime * 100);
  }
  ...
  return {
    ...
    worst_util_percent: round3(worstUtil), packetRows: pktRows, gcl, boardConfigs, stats: outStats
  };
}
...
export function solveGreedy(model) {
  ...
  function findEarliest(lid, earliest, duration) {
    ...
    while (true) {
      let moved = false;
      for (const [s, e] of occ) {
        if (t < e && t + duration > s) { t = e; moved = true; break; }
      }
      if (!moved) return t;
    }
  }

  // Separate TSN and background packets
  const tsnIndices = [], bgIndices = [];
  for (let i = 0; i < pkts.length; i++) {
    if (pkts[i].tsn) tsnIndices.push(i);
    else bgIndices.push(i);
  }

  // Sort TSN packets: highest priority first, then by release time, then by deadline
  const order = tsnIndices.slice();
  order.sort((a, b) => {
  ...
  for (const pi of order) {
    ...
    for (let ri = 0; ri < pk.routes.length; ri++) {
      ...
      for (let h = 0; h < rt.hops.length; h++) {
        const hp = rt.hops[h];
        const s = findEarliest(hp.lid, t, hp.tx);
        if (s + hp.tx > cycleTime) { valid = false; break; }
        starts.push(s);
    ...
    // Commit to link occupancy (no guard — guard bands are added during GCL construction)
    const rt = pk.routes[bestRoute];
    for (let h = 0; h < rt.hops.length; h++) {
      const hp = rt.hops[h];
      linkOcc[hp.lid].push([bestStarts[h], bestStarts[h] + hp.tx]);
    }
  ...
  // Background packets: not scheduled, transmitted in non-ST gaps
  for (const bi of bgIndices) {
    schedHops[bi] = { route: 0, starts: null, skipped: true };
  }
...
export async function solveILP(model, glpk, opts = {}) {
  ...
  if (!model.processing_delay_us) model.processing_delay_us = 3;
  const tmlim = opts.tmlim || 9999999;

  const allPkts = expandPackets(model);

  // Separate TSN and background packets — only TSN enters ILP
  const tsnMap = [];
  const bgIndices = [];
  for (let i = 0; i < allPkts.length; i++) {
    if (allPkts[i].tsn) tsnMap.push(i);
    else bgIndices.push(i);
  }
  const pkts = tsnMap.map(i => allPkts[i]);
  //if (pkts.length > 70) throw new Error(`Too many TSN packets (${pkts.length}). Reduce flows or increase period.`);
  ...
  if (allSingleRoute) {
    ...
    for (let p = 0; p < pkts.length; p++) {
      ...
      for (let h = 0; h < rt.hops.length; h++) {
        ...
        // Tight upper bound
        let latestStart = cycleTime - hp.tx;
        if (pk.dl != null) {
          let tailTime = 0;
    ...
    const M = cycleTime + model.processing_delay_us + 100;
    ...
    for (let p = 0; p < pkts.length; p++) {
      ...
      for (let r = 0; r < pk.routes.length; r++) {
        ...
        for (let h = 0; h < rt.hops.length; h++) {
          ...
          ac('ub', [{ name: s, coef: 1 }, { name: z, coef: M }], { type: glpk.GLP_UP, lb: 0, ub: cycleTime - hp.tx + M });
          if (h < rt.hops.length - 1) {
    ...
    for (const lnk of model.links) {
      ...
      for (let a = 0; a < lo.length; a++) for (let b = a + 1; b < lo.length; b++) {
        ...
        const aEnd = pa.dl ?? cycleTime;
        const bEnd = pb.dl ?? cycleTime;
  ...
  const solved = await glpk.solve(lp, { msglev: glpk.GLP_MSG_ALL, presol: true, tmlim });
  ...
  // Build scheduled hops from ILP solution (TSN only)
  const tsnSchedHops = [];
  if (allSingleRoute) {
    for (let p = 0; p < pkts.length; p++) {
      const starts = [];
      for (let h = 0; h < pkts[p].routes[0].hops.length; h++) starts.push(Number(rv[sv(p, h)] || 0));
      tsnSchedHops.push({ route: 0, starts });
    }
  } else {
    const zv2 = (p, r) => `z_${p}_${r}`;
    for (let p = 0; p < pkts.length; p++) {
      ...
      tsnSchedHops.push({ route: selR, starts });
    }
  }

  // Reassemble schedHops for allPkts (TSN + background)
  const schedHops = new Array(allPkts.length);
  for (let t = 0; t < tsnMap.length; t++) {
    schedHops[tsnMap[t]] = tsnSchedHops[t];
  }
  for (const bi of bgIndices) {
    schedHops[bi] = { route: 0, starts: null, skipped: true };
  }
  ...
  return buildResult(model, allPkts, schedHops,
...
export function renderMetrics(model, result, containerId = "metricsArea") {
  ...
  if (!area) return;
  const tsnPkts = result.packetRows.filter(p => p.status !== "NON-ST");
...
export function renderTopology(model, result, opts = {}) {
  ...
  if (legendEl) {
    ...
    flowTypes.forEach(ft => {
      ...
      legendEl.innerHTML += `<div class="legend-item"><div class="legend-dot" style="background:${c}"></div>${ft} (P${prio?.PCP ?? '?'})</div>`;
    });
    if (!nodeColors) {
...
export function renderGCL(model, result, opts = {}) {
  ...
  if (legendEl) {
    ...
    legendEl.innerHTML = types.map(t => {
      ...
      const c = rep ? colorFn(rep.id) : (FLOW_COLOR_MAP[t] || "#7b61ff");
      return `<div class="legend-item"><div class="legend-dot" style="background:${c}"></div>${t} (P${rep?.PCP ?? '?'})</div>`;
    }).join("") +
  ...
  const activeLinks = model.links.filter(l => {
    ...
    return entries.some(e => !e.note.includes("non-ST") && e.note !== 'BE');
  });
  ...
  activeLinks.forEach(link => {
    ...
    entries.forEach(e => {
      if (!e.note.includes("non-ST") && e.note !== 'BE') maxActiveEnd = Math.max(maxActiveEnd, e.end_us);
    });
  });
  const zoomThreshold = cycleTime * 0.3;
  const isZoomed = maxActiveEnd > 0 && maxActiveEnd < zoomThreshold;
  const xMax = isZoomed ? Math.max(maxActiveEnd * 1.5, maxActiveEnd + 20) : cycleTime;
  ...
  guardPat.append("rect").attr("width", 6).attr("height", 6).attr("fill", "#fef9c3");
  guardPat.append("line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", 6)
    .attr("stroke", "rgba(202,138,4,0.45)").attr("stroke-width", 2);
  ...
  activeLinks.forEach(link => {
    ...
    barG.selectAll("rect.gcl-bar")
      ...
      .attr("class", d => {
        const isMeta = d.note.includes("guard") || d.note.includes("non-ST") || d.note.includes("BE");
        const fid = isMeta ? '' : d.note.split("#")[0];
        return `gcl-bar ${fid ? 'flow-bar' : ''}`
      })
      .attr("data-flow", d => {
        if (d.note.includes("guard") || d.note.includes("non-ST") || d.note.includes("BE")) return '';
      ...
      .attr("fill", d => {
        ...
        if (d.note.includes("non-ST") || d.note.includes("BE")) return beColor;
        return colorFn(d.note);
      })
      .attr("stroke", d => {
        if (d.note.includes("guard")) return "#e8a317";
        if (d.note.includes("non-ST") || d.note.includes("BE")) return beBorder;
        return "rgba(255,255,255,0.4)";
      })
      .attr("stroke-width", d => (d.note.includes("non-ST") || d.note.includes("BE")) ? 0.5 : 0.8)
      .attr("opacity", d => (d.note.includes("non-ST") || d.note.includes("BE")) ? 0.3 : 0.9)
      .attr("rx", 3)
      .attr("filter", d => d.note.includes("non-ST") || d.note.includes("guard") || d.note.includes("BE") ? "none" : "url(#barShadow)")
      .on("mouseover", function(evt, d) {
        const fid = d.note.split("#")[0];
        // Cross-flow highlight: dim all, brighten same flow
        if (!d.note.includes("non-ST") && !d.note.includes("guard") && !d.note.includes("BE")) {
      ...
      .attr("width", d => {
        ...
        return (d.note.includes("non-ST") || d.note === 'BE') ? Math.max(w, 0) : Math.max(w, 3);
      });

    // Gate transition markers — small ticks at flow/guard boundaries
    const transitions = visibleEntries.filter(e => !e.note.includes("non-ST") && e.note !== 'BE');
    ...
    // Labels on flow bars
    barG.selectAll("text")
      .data(visibleEntries.filter(e => !e.note.includes("non-ST") && !e.note.includes("guard") && e.note !== 'BE'))
      .enter().append("text")
  ...
  svg.append("text")
    ...
    .text(`Cycle: ${cycleTime} \u00b5s | Guard: ${model.guard_band_us} \u00b5s | Active: ${activeLinks.length}/${model.links.length} links${isZoomed ? ` | Zoom: 0\u2013${Math.round(xMax)} \u00b5s` : ''}`);

  // Zoom indicator bar
  if (isZoomed) {
    const zoomBarY = H - 14;
    const zoomRatio = xMax / cycleTime;
    svg.append("rect")
...
export function renderUtilization(model, result, optsOrId = "utilContainer") {
  ...
  const linkData = model.links.map(l => {
    ...
    entries.forEach(e => {
      if (e.note.includes("non-ST") || e.note === 'BE') beTime += e.duration_us;
    ...
    return {
      ...
      util: ((flowTime + guardTime) / cycleTime * 100).toFixed(1)
    };
  ...
  linkData.forEach((link, i) => {
    ...
    const data = [
      ...
      { label: "Non-ST", value: link.be, color: beColor }
    ];
    ...
    g.selectAll(".util-arc")
      ...
      .attr("opacity", d => d.data.label === "Non-ST" ? 0.3 : 0.85)
      .on("mouseover", (evt, d) => {
        showTip(evt, `
...
export function renderSwitchGCL(model, result, opts = {}) {
  ...
  switches.forEach(sw => {
    model.links.filter(l => l.from === sw.id).forEach(l => {
      const entries = result.gcl.links[l.id]?.entries || [];
      entries.forEach(e => {
        if (!e.note.includes("non-ST") && e.note !== 'BE') globalMaxActive = Math.max(globalMaxActive, e.end_us);
      });
    });
  });
  const zoomThreshold = cycleTime * 0.3;
  const isZoomed = globalMaxActive > 0 && globalMaxActive < zoomThreshold;
  const xMax = isZoomed ? Math.max(globalMaxActive * 1.5, globalMaxActive + 20) : cycleTime;
  ...
  switches.forEach(sw => {
    ...
    activeEgress.forEach(l => {
      ...
      entries.forEach(e => {
        ...
        else if (!e.note.includes("non-ST")) { totalFlow += e.duration_us; tsnCount++; }
      });
    });
    const utilPct = ((totalFlow + totalGuard) / cycleTime / activeEgress.length * 100).toFixed(1);
    ...
    activeEgress.forEach(link => {
      ...
      visibleEntries.forEach(e => {
        const isBE = e.note.includes("non-ST");
      ...
      visibleEntries.forEach(e => {
        if (e.note.includes("non-ST")) return;
